    ...
    return {
        ...
        respond: function( data ){
            ...
            var self = this
                ...
                ,targets = this.getTargets() // Hart Custom bullshit
                ,i,l,b
                ;
            ...
            for ( i = 0, l = collisions.length; i < l; ++i ){
                col = collisions[ i ];
                // Hart Custom bullshit
                // Implementing applyTo, only apply impulse response if both bodies are valid targets
                if (targets !== this._world._bodies && 
                  (Physics.util.indexOf( targets, col.bodyA ) === -1) || 
                  (Physics.util.indexOf( targets, col.bodyB ) === -1)
                ){continue}
...
Physics.renderer('pixi', function( parent ){
    ...
    return {
        ...
        drawBody: function( body, view ){
          ...
          var offset = {x:0,y:0};
          if(body.follow != null){
            var target = body.follow.body;
            offset.x = target.state.pos.x + target.state.vel.x * t;
            offset.y = 0;//target.state.pos.y + target.state.vel.y * t;
            view.position.set( worldWidth/4 + (x - offset.x), y );
          }else{
            view.position.set( x , y );
          }
          
          
            //view.position.set( worldWidth/2 + (x - offset.x), y );
